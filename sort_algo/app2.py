...
class NodeApp(ctk.CTk):
    ...
    def get_facing_side_center(self, node, x, y):
        """ Get the center of the side facing another node. """
        coords = self.canvas.coords(node)
        x1, y1, x2, y2 = coords
        ...
        # Determine which side is facing the other node
        if x < node_center_x:
            # Left side
            return x1, node_center_y
        elif x > node_center_x:
            # Right side
            return x2, node_center_y
        elif y < node_center_y:
            # Top side
            return node_center_x, y1
        else:
            # Bottom side
            return node_center_x, y2
    ...
    def save_state(self):
        ...
        for node_id, node_data in self.nodes.items():
            ...
            for target_node, line_id in node_data["connections"]:
    ...
    def load_state(self):
        try:
            ...
            for node_id, node_info in data["nodes"].items():
                ...
                node_id = self.canvas.create_rectangle(*coords, fill="#333333", outline="#555555", width=4, tags=("node",))
                text_id = self.canvas.create_text((coords[0] + coords[2]) / 2, (coords[1] + coords[3]) / 2,
                                                  text=node_info["name"], fill="#ffffff", font=("Arial", 10, "bold"), tags=("node_text",))
                self.nodes[node_id] = {"name": node_info["name"], "text": node_info["text"], "text_id": text_id, "connections": []}

            for line_info in data["lines"]:
                line = self.canvas.create_line(*line_info["coords"], fill="yellow", width=2, smooth=True, arrow=tk.LAST)
                self.nodes[line_info["start"]]["connections"].append((line_info["end"], line))
                self.nodes[line_info["end"]]["connections"].append((line_info["start"], line))
    ...
    def update_node_title(self, event=None):
        if self.current_node:
            title = self.node_title.get()
            self.nodes[self.current_node]["name"] = title
            text_id = self.nodes[self.current_node]["text_id"]
            self.canvas.itemconfig(text_id, text=title)

    def update_node_text(self, event=None):
        if self.current_node:
            text = self.node_text.get("1.0", tk.END).strip()
            self.nodes[self.current_node]["text"] = text
    ...
    def delete_node(self, event=None):
        if self.current_node:
            # Delete connections
            for connected_node, line_id in self.nodes[self.current_node]["connections"]:
                self.canvas.delete(line_id)
                self.nodes[connected_node]["connections"] = [conn for conn in self.nodes[connected_node]["connections"] if conn[1] != line_id]
            # Delete node and text
            ...
            self.current_node = None
            self.sidebar.place_forget()  # Hide the sidebar
